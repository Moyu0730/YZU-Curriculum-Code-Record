...
// product = multiplicand * multiplier
void multiplication( HugeInt multiplicand, HugeInt multiplier, HugeInt &product ){
    if( ( multiplicand.size == 1 && multiplicand.digit[ 0 ] == 0 ) || ( multiplier.size == 1 && multiplier.digit[ 0 ] == 0 ) ){
        product.size = 1;
    ...
    product.size = multiplicand.size + multiplier.size + 1;
    ...
    for( int i = 0 ; i < multiplier.size ; i++ ){
        for( int j = 0 ; j < multiplicand.size ; j++ ){
            product.digit[i+j] += multiplicand.digit[j] * multiplier.digit[i];
        }
    }
