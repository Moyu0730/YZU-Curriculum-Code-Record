...
public:
   ...
   {
       if (right.size() != 0) {
           myData.myFirst = new value_type[right.capacity() + 1]();
   ...
   iterator insert( const_iterator where, const value_type &val )
   {
      ...
      size_type idx = where - myData.myFirst;
      ...
      if (originalSize == originalCapacity) {
          ...
          for (size_type i = originalSize; i > idx; --i) {
   ...
   vector& operator=( const vector &right )
   {
      if( this != &right ) // avoid self-assignment
      {
         ...
         if( rightSize > capacity() )
         {
            ...
            myData.myFirst = new value_type[newCapacity + 1]();
			myData.myEnd = myData.myFirst + newCapacity;
         }

         for (size_type i = 0; i < rightSize; ++i) myData.myFirst[i] = right.myData.myFirst[i];

		 myData.myLast = myData.myFirst + rightSize;
         myData.myFirst[rightSize] = value_type();
      }
