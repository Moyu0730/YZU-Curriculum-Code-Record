...
public:
      ...
      : myData()
   {
      ...
      else
      {
          ...
          myData.mapSize = 8;
          while (count > dequeSize * (myData.mapSize - 1)) myData.mapSize *= 2;

          myData.map = new value_type * [myData.mapSize]();
          myData.myOff = 0;

          int finished = 0;
          while (finished * dequeSize < count) {
              size_t remain = std::min(dequeSize, count - finished * dequeSize);
              myData.map[finished] = new value_type[dequeSize];

              for (size_t i = 0; i < remain; ++i) myData.map[finished][i] = val;
