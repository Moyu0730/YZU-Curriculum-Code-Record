...
public:
      ...
      : myData()
   {
      if( right.myData.mySize > 0 )
      {
          ...
          myData.mapSize = 8;
          while (myData.mySize > dequeSize * (myData.mapSize - 1)) myData.mapSize *= 2;

          myData.map = new value_type * [myData.mapSize]();
          myData.myOff = 0;

          int Finished = 0, rightFinished = right.myData.myOff;
          while (Finished * dequeSize < right.myData.mySize) {
              myData.map[Finished] = new value_type[dequeSize];

              size_t remain = std::min(dequeSize, right.myData.mySize - Finished * dequeSize);
              for (size_t i = 0; i < remain; ++i, ++rightFinished) {
                  size_t rightI = (rightFinished / dequeSize) % right.myData.mapSize;
                  size_t rightJ = rightFinished % dequeSize;

                  myData.map[Finished][i] = right.myData.map[rightI][rightJ];
              }
