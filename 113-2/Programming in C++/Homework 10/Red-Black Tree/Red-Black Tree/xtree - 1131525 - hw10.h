...
public:
   ...
   void rightRotation( TreeNode< value_type > *g ){
      ...
      ////////////////////////////////////////////////// Start
      TreeNode< value_type > *pR = p->right;
   ...
   void eraseDegreeOne( TreeNode< value_type > *erasedNode )
   {
      ...
      if( erasedNode == myHead->parent ) // erasedNode points to the root; Cases 2 & 3 in "Ch 3 Sec 9.pptx"
      {
          ...
          if (erasedNode->left != myHead ) {
              ...
              erasedNode->left->parent = myHead;
              erasedNode->left->isNil = 1;
          }
          else if (erasedNode->right != myHead) {
              erasedNode = erasedNode->right;
              erasedNode->right->parent = myHead;
              erasedNode->right->isNil = 1;
          }
          else {
              myHead->left = myHead;
              myHead->parent = myHead;
      ...
      else
      {
          ...
          if (erasedNode->left != myHead) {
              ...
              if (erasedNode == erasedNode->parent->left) erasedNode->parent->left = erasedNode->left;
              else erasedNode->parent->right = erasedNode->left;
          }
          else if (erasedNode->right != myHead) {
              erasedNode->right->parent = erasedNode->parent;

              if (erasedNode == erasedNode->parent->left) erasedNode->parent->left = erasedNode->right;
              else erasedNode->parent->right = erasedNode->right;
          }
          else {
              if (erasedNode == erasedNode->parent->left) erasedNode->parent->left = myHead;
              else erasedNode->parent->right = myHead;
          }
          ////////////////////////////////////////////////// End
